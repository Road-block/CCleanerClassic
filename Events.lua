...
local WELCOME_MSG_DELAY = 10
addon.GetAddOnMetadata = C_AddOns and C_AddOns and C_AddOns.GetAddOnMetadata or _G.GetAddOnMetadata
...
local function CreateInterfaceOptionsEntry()
	...
	txt:SetText(string.format("%s v%s", addon.GetAddOnMetadata(addonName, "Title"), addon.GetAddOnMetadata(addonName, "Version")));
	...
	txt:SetText(string.format("/%s", addon.GetAddOnMetadata(addonName, "Title")));
...
local function EventHandler(self, event, sender, arg1, ...)
	...
	if (event == "ADDON_LOADED") and (sender == addonName) then
